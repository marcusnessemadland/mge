...
namespace ImGui
{
    ...
    IMGUI_API void          TextLinkOpenURL(const char* label, const char* url = nullptr);     // hyperlink text button, automatically open file/url when clicked
    ...
    IMGUI_API void          Columns(int count = 1, const char* id = nullptr, bool borders = true);
...
enum ImGuiTabBarFlags_
{
    ...
    ImGuiTabBarFlags_NoCloseWithMiddleMouseButton   = 1 << 3,   // Disable behavior of closing tabs (that are submitted with p_open != nullptr) with middle mouse button. You may handle this behavior manually on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = false.
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_NoCloseWithMiddleMouseButton  = 1 << 2,   // Disable behavior of closing tabs (that are submitted with p_open != nullptr) with middle mouse button. You may handle this behavior manually on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = false.
...
struct ImGuiSelectionBasicStorage
{
    ...
    IMGUI_API bool  GetNextSelectedItem(void** opaque_it, ImGuiID* out_id); // Iterate selection with 'void* it = nullptr; ImGuiId id; while (selection.GetNextSelectedItem(&it, &id)) { ... }'
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddText(ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = nullptr, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = nullptr);
...
struct ImFontAtlasCustomRect
{
    ...
    ImFontAtlasCustomRect()         { X = Y = 0xFFFF; Width = Height = 0; GlyphID = 0; GlyphColored = 0; GlyphAdvanceX = 0.0f; GlyphOffset = ImVec2(0, 0); Font = nullptr; }
...
struct ImFont
{
    ...
    IMGUI_API ImVec2            CalcTextSizeA(float size, float max_width, float wrap_width, const char* text_begin, const char* text_end = nullptr, const char** remaining = nullptr); // utf8
