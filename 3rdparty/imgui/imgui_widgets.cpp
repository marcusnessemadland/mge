...
static ImVec2   InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin, const char* text_end, const char** remaining = nullptr, ImVec2* out_offset = nullptr, bool stop_on_new_line = false);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!is_indeterminate || overlay != nullptr)
    {
        ...
        if (overlay_size.x > 0.0f)
        {
            ...
            RenderTextClipped(ImVec2(ImClamp(text_x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x), bb.Min.y), bb.Max, overlay, nullptr, &overlay_size, ImVec2(0.0f, 0.5f), &bb);
        }
    }
}
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        ...
        if ((flags & ImGuiSliderFlags_ClampOnInput) && (p_min != nullptr || p_max != nullptr))
        {
            const int clamp_range_dir = (p_min != nullptr && p_max != nullptr) ? DataTypeCompare(data_type, p_min, p_max) : 0; // -1 when *p_min < *p_max, == 0 when *p_min == *p_max
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step == nullptr)
    {
        if (InputText(label, buf, IM_ARRAYSIZE(buf), flags))
            value_changed = DataTypeApplyFromText(buf, data_type, p_data, format, (flags & ImGuiInputTextFlags_ParseEmptyRefVal) ? p_data_default : nullptr);
    }
    else
    {
...
static void    STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj, int line_start_idx)
{
    ...
    const ImVec2 size = InputTextCalcTextSize(obj->Ctx, text + line_start_idx, text + obj->TextLen, &text_remaining, nullptr, true);
    r->x0 = 0.0f;
    r->x1 = size.x;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    else if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (recycle_state && (state->TextLen != buf_len || (state->TextA.Data == nullptr || strncmp(state->TextA.Data, buf, buf_len) != 0)))
            recycle_state = false;
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                {
                    unsigned int c;
                    int in_len = ImTextCharFromUtf8(&c, s, nullptr);
                    s += in_len;
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            const char* cursor_ptr = render_cursor ? text_begin + state->Stb->cursor : nullptr;
            const char* selmin_ptr = render_selection ? text_begin + ImMin(state->Stb->select_start, state->Stb->select_end) : nullptr;

            // Count lines and find line number for cursor and selection ends
            int line_count = 1;
            if (is_multiline)
            {
                for (const char* s = text_begin; (s = (const char*)memchr(s, '\n', (size_t)(text_end - s))) != nullptr; s++)
                {
        ...
        if (render_selection)
        {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                {
                    ImVec2 rect_size = InputTextCalcTextSize(&g, p, text_selected_end, &p, nullptr, true);
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            if (flags & ImGuiInputTextFlags_ElideLeft)
                draw_pos.x = ImMin(draw_pos.x, frame_bb.Max.x - CalcTextSize(buf_display, nullptr).x - style.FramePadding.x);
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect()
{
    ...
    IM_ASSERT(g.CurrentMultiSelect != nullptr && storage->Window == g.CurrentWindow);
...
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO* ms_io)
{
    ...
    IM_ASSERT(AdapterIndexToStorageId != nullptr);
