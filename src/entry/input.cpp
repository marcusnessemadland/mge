...
#include "entry_p.h"
#include "vulkan-renderer/input.h"
#include "vulkan-renderer/cmd.h"
...
namespace vr
{
	struct InputMouse
	{
		...
		void setButtonState(vr::MouseButton::Enum _button, uint8_t _state)
		{
		...
		float m_norm[3];
		int32_t m_wheel;
		uint8_t m_buttons[vr::MouseButton::Count];
		uint16_t m_width;
		uint16_t m_height;
	...
	struct InputKeyboard
	{
		...
		void setKeyState(vr::Key::Enum _key, uint8_t _modifiers, bool _down)
		{
			m_key[_key] = encodeKeyState(_modifiers, _down);
			m_once[_key] = false;
		}

		bool getKeyState(vr::Key::Enum _key, uint8_t* _modifiers)
		{
			uint8_t modifiers;
		...
		{
			uint8_t modifiers = 0;
			for (uint32_t ii = 0; ii < vr::Key::Count; ++ii)
			{
	...
	struct Gamepad
	{
		...
		void setAxis(vr::GamepadAxis::Enum _axis, int32_t _value)
		{
			m_axis[_axis] = _value;
		}

		int32_t getAxis(vr::GamepadAxis::Enum _axis)
		{
			return m_axis[_axis];
		}

		int32_t m_axis[vr::GamepadAxis::Count];
	};
	...
	struct Input
	{
		...
		{
			for (const InputBinding* binding = _bindings; binding->m_key != vr::Key::None; ++binding)
			{
				uint8_t modifiers;
	...
	void inputInit()
	{
		s_input = BX_NEW(vr::getAllocator(), Input);
	}

	void inputShutdown()
	{
		bx::deleteObject(vr::getAllocator(), s_input);
	}
	...
	bool inputGetKeyState(vr::Key::Enum _key, uint8_t* _modifiers)
	{
	...
	void inputSetMouseLock(bool _lock)
	{
		if (s_input->m_mouse.m_lock != _lock)
		{
			...
			vr::setMouseLock(vr::kDefaultWindowHandle, _lock);
			if (_lock)
			{
				s_input->m_mouse.m_norm[0] = 0.0f;
				s_input->m_mouse.m_norm[1] = 0.0f;
