...
namespace vr
{
    EventQueue::EventQueue(Window* _window) 
        : processingMode(ProcessingMode::Poll)
    { 
        initialized = false; 

        _window->mEventQueue = this;
    }
    ...
    void EventQueue::setProcessingMode(ProcessingMode::Enum mode)
    {
        processingMode = mode;
    }
    ...
    LRESULT EventQueue::pushEvent(MSG msg, Window* window)
    {
        ...
        case WM_PAINT:
        {
            ...
            uint32_t bg = window->getBackgroundColor();
            uint32_t r = (bg & 0xff000000) >> 24;
            uint32_t g = (bg & 0x00ff0000) >> 16;
            uint32_t b = (bg & 0x0000ff00) >> 8;
        ...
        case WM_XBUTTONDOWN:
        {
            ...
            e = Event(
                MouseData(
                    x & XBUTTON1 ? Mouse::Button4 : Mouse::Button5,
                    ButtonState::Pressed,
                    ModifierState(modifiers & MK_CONTROL, modifiers & MK_ALT,
                                        modifiers & MK_SHIFT, modifiers & 0)),
                window);
        ...
        case WM_MOUSEMOVE:
        {
            ...
            area.bottom))
            {
                ...
                if (m_mouseInside)
                {
                    ...
                    // No uint64_ter care for the mouse leaving the window
                    setTracking(false);
            ...
            e = Event(
                MouseMoveData(
                    static_cast<uint32_t>(area.left <= x && x <= area.right
                                              ? x - area.left
                                              : 0xFFFFFFFF),
                    static_cast<uint32_t>(area.top <= y && y <= area.bottom
                                              ? y - area.top
                                              : 0xFFFFFFFF),
                ...
                window);
            prevMouseX = static_cast<uint32_t>(x);
            prevMouseY = static_cast<uint32_t>(y);
            break;
        }
        case WM_KEYDOWN:
        case WM_KEYUP:
        ...
        case WM_SIZE:
        {
            ...
            width = static_cast<uint32_t>((UINT64)msg.lParam & 0xFFFF);
            height = static_cast<uint32_t>((UINT64)msg.lParam >> 16);
        ...
        case WM_GETMINMAXINFO:
        {
            ...
            min_max->ptMinTrackSize.x = window->mDesc.minWidth;
            min_max->ptMinTrackSize.y = window->mDesc.minHeight;
            break;
        }
        default:
            // Do nothing
    ...
    {
        return std::make_shared<EventQueue>(_window.get());
    }
